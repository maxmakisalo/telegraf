...
func (s *HuaweiApi) Description() string {
	return "Return huawei API data"
}
...
func (s *HuaweiApi) SampleConfig() string {
	return sampleConfig
}
...
func (p *HuaweiApi) GetFields(data map[string]interface{}, module Module) (map[string]interface{}, map[string]string, error) {
...
func (p *HuaweiApi) ConcatTags(tag map[string]string, tags []map[string]string) map[string]string {
...
func (p *HuaweiApi) Gather(acc telegraf.Accumulator) error {
		...
		go func(address string) {
			...
			if err != nil {
				...
				for _, table := range tables {
					...
					tag["address"] = address
					acc.AddFields("huawei_api", table.Fields, tag)
				}
			}
		}(address)
	}
...
func init() {
	inputs.Add("huawei_hilink_api", func() telegraf.Input {
		return &HuaweiApi{}
	})
}
